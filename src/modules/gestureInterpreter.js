...
export class GestureInterpreter {
  constructor() {
    ...
    // Secondary hand smoothing
    this._lastControlGesture = CONTROL_GESTURES.IDLE;
    this._idleFrameCount = 0;
    this._idleFramesRequired = 4;

    // Primary hand smoothing: evita que DRAW quebre por frames de IDLE espúrios
    this._lastPrimaryGesture = GESTURES.IDLE;
    this._primaryIdleFrameCount = 0;
    this._primaryIdleFramesRequired = 12;

    // Persistência da mão: usa os últimos landmarks quando a mão some por poucos frames
    this._lastPrimaryLandmarks = null;
    this._noHandFrameCount = 0;
    this._noHandFramesRequired = 15;
  }
  ...
  interpret(results) {
    ...
    if (!results.multiHandLandmarks || results.multiHandLandmarks.length === 0) {
      // Mão sumiu — usa os últimos landmarks por até N frames para não quebrar o traço
      if (this._lastPrimaryLandmarks && this._noHandFrameCount < this._noHandFramesRequired) {
        this._noHandFrameCount++;
        // Roda o processamento com os últimos landmarks conhecidos
        const rawPrimary = this.primaryController.update(this._lastPrimaryLandmarks);
        if (rawPrimary !== GESTURES.IDLE) {
          output.primary.gesture = rawPrimary;
          output.primary.landmark = this._lastPrimaryLandmarks[8];
          output.primary.fingertips = [4, 8, 12, 16, 20].map(i => this._lastPrimaryLandmarks[i]);
        }
        return output;
      }
      this._lastPrimaryLandmarks = null;
    ...
    if (primaryLandmarks) {
      const rawPrimary = this.primaryController.update(primaryLandmarks);

      // Suavização: só troca para IDLE após N frames consecutivos sem gesto
      if (rawPrimary === GESTURES.IDLE) {
        this._primaryIdleFrameCount++;
        if (
          this._primaryIdleFrameCount < this._primaryIdleFramesRequired &&
          this._lastPrimaryGesture !== GESTURES.IDLE
        ) {
          output.primary.gesture = this._lastPrimaryGesture;
        } else {
          output.primary.gesture = GESTURES.IDLE;
          this._lastPrimaryGesture = GESTURES.IDLE;
        }
      } else {
        this._primaryIdleFrameCount = 0;
        this._lastPrimaryGesture = rawPrimary;
        output.primary.gesture = rawPrimary;
      }
      ...
      this._lastPrimaryLandmarks = primaryLandmarks;
    }
